...
def Hamiltonian(h, rho, theta): 
	...
	H = np.sum( (D*lap_rho + rho -rho**3 + kappa*np.outer(np.mean(rho**2,axis=1), np.ones(L)))*theta + 0.5*aa*theta**2,  axis=1)
	return H
...
for i in range(0, iterations+1):
	...
	lap_rho = lap(rho)
	lap_theta = lap(theta)
	dH_drho = D*(lap_theta)+ theta - 3*rho**2 *theta + kappa*2*rho*np.outer(np.mean(theta, axis=1), np.ones(L))
	# dH_drho = -D * lap(lap_theta) - (1 - 3*rho**2) * lap_theta
	
	dH_dtheta = D*(lap_rho) + rho -rho**3 + kappa*np.outer(np.mean(rho**2,axis=1), np.ones(L)) + aa*theta
	...
	if( i%plotStep == 0):
		...
		if(upward==True):
			plt.savefig('upward_L'+str(int(L))+'_N'+str(int(Ncopy))+'h'+str(int(h))+'_D'+str(D)+'_dtau'+str("%.1e"%dtau)+'_'+"%09d" % (i,)+'.png', format='png', bbox_inches='tight')
		else:
			plt.savefig('downward_L'+str(int(L))+'_N'+str(int(Ncopy))+'h'+str(int(h))+'_D'+str(D)+'_dtau'+str("%.1e"%dtau)+'_'+"%09d" % (i,)+'.png', format='png', bbox_inches='tight')
		plt.clf()
		plt.close()
