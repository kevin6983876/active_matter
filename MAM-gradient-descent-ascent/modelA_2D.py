...
def Hamiltonian(h, rho, theta): 
    ...
    lap_rho_3d = apply_lap_2d(rho_3d)
    mean_rho2_global = np.mean(rho_3d**2, axis=(1, 2), keepdims=True)
    H_density = (D * lap_rho_3d + rho_3d - rho_3d**3 + kappa * mean_rho2_global) * theta_3d + 0.5 * aa * theta_3d**2
    H = np.sum(H_density, axis=(1, 2))
    return H
...
for i in range(start_iter, iterations+1):
	...
	if i > start_iter:
		...
		H_path = Hamiltonian(h, rho, theta).real*h**2*aa
...
def animate_2d_heatmap(rho, filename="2d_evolution.mp4", dnu=None, skip=1, fps=30):
    ...
    # Initial Plot
    # origin='lower' 讓 (0,0) 在左下角
    im = ax.imshow(rho_sub[0], cmap='coolwarm', vmin=-1.5, vmax=1.5, 
    ...
    def update(frame_idx):
        # Update data
        im.set_data(rho_sub[frame_idx])
        
        # Update Title
        t = indices[frame_idx] * dnu
        title.set_text(f"Time: {t:.2f} | Path Evolution")
        return im, title

    anim = FuncAnimation(fig, update, frames=n_frames, blit=True, interval=1000.0/fps)
...
animate_2d_heatmap(rho.real, filename=f"2d_sim_Lx{Lx}_Ly{Ly}.mp4", dnu=dnu, skip=4, fps=30)
