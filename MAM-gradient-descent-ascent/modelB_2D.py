...
d = 40 # distance from the center of the initial state to the center of the final state
...
upward = False # True if droplet to stripe, False if stripe to droplet
D     = 1
...
# 將距離攤平排序，嚴格挑選前 target_area 個最靠近中心的點
...
plt.imshow(rho1.real, cmap='bwr', vmin=-1.5, vmax=1.5, origin='lower', aspect='auto')
plt.colorbar()
plt.savefig('rho1.png')
plt.imshow(rho2.real, cmap='bwr', vmin=-1.5, vmax=1.5, origin='lower', aspect='auto')
...
for i in range(start_iter, iterations+1):
	...
	if i > start_iter:
		...
		if max_diff_rho < 5e-5 and H_std < Hamiltonian(h,rho_1d_check, theta_1d_check).real[2:-10].max()*h**2*aa*1e-2:
			print(f"\n✨ 恭喜！系統在第 {i} 步已完美收斂抵達 Instanton 鞍點！")
			print(f"最大數值誤差 dRho: {max_diff_rho:.2e} | 物理守恆 H 標準差: {H_std:.2e}")
			break  # 安全跳出，完美存檔！
		# 判定條件：
        # 1. 絕對閾值放寬：只要 dRho 進入 10^-3 量級，且 H_std 夠小，就代表進入了鞍點區域。
        # 2. 動態偵測：如果當前的 max_diff_rho 比上一次紀錄的「還要大」，代表系統已經觸底反彈，正在繞極限環！
        
        # (在迴圈外、start_iter 前面，記得先宣告 prev_max_diff = float('inf') )
        
		# if max_diff_rho < 5e-4 and H_std < 0.05:
		# 	# 檢查是否觸底反彈 (Tolerance 可以抓一點緩衝)
		# 	if max_diff_rho > prev_max_diff * 1.5: 
		# 		print(f"\n✨ 恭喜！偵測到 GDA 鞍點極限環，系統在第 {i} 步已完美觸底收斂！")
		# 		print(f"極限誤差 dRho: {max_diff_rho:.2e} | 物理底噪 H 標準差: {H_std:.2e}")
		# 		break  # 安全跳出，完美存檔！
            
		prev_max_diff = max_diff_rho  # 更新紀錄		
	...
	if( i%plotStep == 0):
		if i > start_iter:
			print('max_diff_rho, H_std, H_max:', max_diff_rho, H_std, Hamiltonian(h,rho_1d_check, theta_1d_check).real[2:-10].max()*h**2*aa*1e-2)
		...
		im2 = ax2.pcolormesh(t_edges, y_edges, theta_slice, 
							cmap='bwr', 
							shading='flat',
							vmin=theta_slice.min(), vmax=theta_slice.max())
		...
		ax3.set_aspect('auto')
		ax3.plot(np.linspace(0,1,Ncopy), Lag*h**2*aa, label=r'$L(\rho,\dot\rho)$', color='black'  ) 
		# ax3.plot(np.linspace(0,1,Ncopy), Lag_theo*h**2, label=r'$L_{th}(\rho,\dot\rho)$', color='red' , linestyle='--') 
		ax3.plot(np.linspace(0,1,Ncopy), Hamiltonian(h,rho_1d, theta_1d).real*h**2*aa, label=r'$H(\rho,\theta)$', color='brown' , linestyle='-.') 
		plt.legend(loc='best', fontsize=14)
		ax3.tick_params(axis='both', which='major', labelsize=15)
		ax3.set_xlabel(r'$t/T_\mathrm{Max}$', fontsize=20)
		ax3.set_ylabel(r'$L$', fontsize=20)
		ax3.set_xlim(0,1)
		ax3.set_ylim(-0.01*max(Lag[2:-10].max()*h**2*aa, Hamiltonian(h,rho_1d, theta_1d).real[2:-10].max()*h**2*aa), max(Lag[2:-10].max()*h**2*aa, Hamiltonian(h,rho_1d, theta_1d).real[2:-10].max()*h**2*aa)*1.2)
