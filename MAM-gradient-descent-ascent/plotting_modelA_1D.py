...
resume_file = "checkpoints/checkpoint12.npz"
...
fig.suptitle(r'$N_t=$'+str(int(Ncopy))+r', $N_x*N_y=$'+str(Ly)+r'$\times$'+str(Lx)+r'$, h =$'+str(h)+r', $\Delta \tau=$'+str(dtau)+r', $D=$'+str(D) +r', $T_\mathrm{max}=$'+str(Tmax)+"\n"+"$\kappa =$"+str(kappa)+', Time '+ str(end_iterations*dtau), fontsize=20)
...
ax3.set_aspect('auto')
ax3.plot(np.linspace(0,1,Ncopy), Lag*h*aa, label=r'$L(\rho,\dot\rho)$'+r', $S=$'+str("%.6f"%(actionS)), color='black'  ) 
