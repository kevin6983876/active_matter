...
r = dtau/dnu
resume_file = "KHcheckpoints/checkpoint.npz"
relaxed_file = 'KHcheckpoints/relaxed4.npz'
...
if os.path.exists(relaxed_file):
	...
	if os.path.exists(f'local.txt'):
		...
		fig_states, axs = plt.subplots(2, 3, figsize=(15, 8), constrained_layout=True)
...
if previous_data == True:
	if os.path.exists('KHcheckpoints/checkpoint1_6.npz'):
		data = np.load('KHcheckpoints/checkpoint1_6.npz')
		rho_old = data['rho']
		m_old = data['m']
...
for i in range(start_iter, iterations+1):
	...
	dH_dpm = -apply_grad_y(bracket_2) - 2.0*m_safe + 4.0*rho_safe*p_m

	# δH/δρ
	dH_drho = lap_pr- Pe * m_safe * pr_prime+ Pe * (1.0 - 2.0 * rho_safe) * pm_prime+ (1.0 - 2.0 * rho_safe) * pr_prime**2- 2.0 * m_safe * pr_prime * pm_prime+ 2*p_m**2+pm_prime**2

	# δH/δm
	dH_dm = lap_pm+ Pe * (1.0 - rho_safe) * pr_prime- 2.0 * p_m+ 2.0 * (1.0 - rho_safe) * pr_prime * pm_prime-2.0*m_safe*pm_prime**2
